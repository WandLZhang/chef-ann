...
GCP_LOCATION = os.environ.get(
    "GCP_LOCATION", "global"
)  # gemini-3.1-pro-preview is in 'global' region
...
logger.info(
    f"Loaded {len(COMMODITIES_COMPREHENSIVE.get('all_products', []))} comprehensive products"
)
...
CATEGORY_MAP = {
    ...
    "dairy": [
        ...
        "cheese",
        "eggs",
    ],  # Merge yogurts + cheeses + eggs into one frontend tab
    "legumes": [
        "beans"
    ],  # Includes peanut butter, sunflower butter (credit as meat_alternate)
}
...
def handle_commodities(category=None):
    ...
    if category:
        if category in CATEGORY_MAP:
            ...
            return (
                jsonify(
                    {
                        "error": f"Category '{category}' not found. Valid: {list(CATEGORY_MAP.keys())}"
                    }
                ),
                404,
                cors_headers(),
    ...
    summary = {
        slug: len(sum([comprehensive_cats.get(c, []) for c in comp_cats], []))
...
def handle_stream_allocate(data):
    ...
    """
    _commodity_type = data.get(
        "commodity_type", "beef"
    )  # reserved for future per-type logic
    ...
    for item in items:
        ...
        commodity = next(
            (
                c
                for c in comprehensive_products
                if str(c.get("wbscm_id", "")) == wbscm_id
            ),
            None,
        )
    ...
    if not items_data:
        ...
        return Response(
            error_gen(), mimetype="text/event-stream", headers=cors_headers()
        )
...
"""
    ...
    return Response(
        stream_gemini(prompt, enable_code_execution=True),
...
def handle_stream_compliance(data):
    ...
    meal_requirements = MEAL_PATTERNS.get(
        grade_group, MEAL_PATTERNS.get("elementary", {})
    )
...
"""
    ...
    return Response(
        stream_gemini(prompt, enable_code_execution=enable_code),
        mimetype="text/event-stream",
        headers=headers,
    )
